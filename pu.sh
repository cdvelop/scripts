...
# Verifica si el commit más reciente ha sido empujado
ahead_count=$(git rev-list --count HEAD..@{u} 2>/dev/null)

if [ $? -ne 0 ]; then
    echo "La rama no tiene upstream configurado."
    ahead_count=1
fi

if [ "$ahead_count" -gt 0 ]; then
    echo "Hay commits sin empujar, se realizará el push."
    ...
    echo "No hay commits que empujar, todo está sincronizado con el remoto."
fi
